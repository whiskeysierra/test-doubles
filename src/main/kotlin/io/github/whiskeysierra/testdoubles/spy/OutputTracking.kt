...
class OutputTracking<Output> {
    ...
    fun add(output: Output): Unit =
        subscriptions.forEach { it.add(output) }

    fun track(): Tracker<Output> =
        Subscription(this).also(subscriptions::add)

    internal fun remove(subscription: Tracker<Output>) =
